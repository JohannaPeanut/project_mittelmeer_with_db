...
var Beta = /*#__PURE__*/function () {
  function Beta(alpha, beta) {
    ...
    this.beta = Number(beta); // Create an instance of Gamma distribution class for alpha

    this.gammaA = new Gamma(this.alpha, 1); // Create an instance of Gamma distribution class for beta
  ...
  _createClass(Beta, [{
    ...
    value: function isError() {
      if (!this.alpha || !this.beta) {
        return {
          error: 'Beta distribution: you should point "alpha" and "beta" positive numerical values'
        };
      }

      if (this.alpha <= 0) {
        return {
          error: 'Beta distribution: Parameter "alpha" must be positive'
        };
      }

      if (this.beta <= 0) {
        return {
          error: 'Beta distribution: Parameter "beta" must be positive'
        };
      }
    ...
    value: function toString() {
      var info = ['Beta Distribution', "Usage: unirand.beta(".concat(this.alpha, ", ").concat(this.beta, ").random()")];
      return info.join('\n');
    }
    /**
     * Mean value
